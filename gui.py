...
class VulnerabilitiesGUI:
    ...
    def crear_frame_scrolleable(self, parent):
        """Crea un frame scrolleable con canvas y scrollbars"""
        ...
        # Frame interno que contendrá el contenido con padding
    ...
    def setup_phase_discovery(self):
        ...
        self.notebook.add(container, text="Fase 1: Discovery")
        container.columnconfigure(0, weight=1)
        container.rowconfigure(0, weight=1)
        
        frame, canvas = self.crear_frame_scrolleable(container)
    ...
    def setup_phase_versions(self):
        ...
        container = ttk.Frame(self.notebook)
        self.notebook.add(container, text="Fase 2: Versiones")
        container.columnconfigure(0, weight=1)
        container.rowconfigure(0, weight=1)
    ...
    def setup_phase_excel(self):
        ...
        container = ttk.Frame(self.notebook)
        self.notebook.add(container, text="Fase 3: Excel Final")
        container.columnconfigure(0, weight=1)
        container.rowconfigure(0, weight=1)
    ...
    def generar_excel_final(self):
        ...
        try:
            ...
            carpeta_excel = guardar_xlsx_completo(
                ...
                self.identificador,
                archivos_todos,
                vulnerabilidades_ia=self.vulnerabilidades_detectadas if hasattr(self, 'vulnerabilidades_detectadas') else None
            )
    ...
    def abrir_carpeta_resultados(self):
        ...
        if not self.carpeta_resultados or not os.path.exists(self.carpeta_resultados):
            messagebox.showwarning(
                "Carpeta no disponible",
                "No hay carpeta de resultados disponible. Genera el Excel primero."
            )
            return
        ...
        try:
            ...
            if sistema == "Windows":
                ...
                subprocess.run(["open", self.carpeta_resultados])
            else:  # Linux y otros
                subprocess.run(["xdg-open", self.carpeta_resultados])
            
            self.log(f"📂 Carpeta abierta: {self.carpeta_resultados}")
        except Exception as e:
            self.log(f"❌ Error al abrir carpeta: {str(e)}")
...
Total: {resumen.get('total', 0)} | Críticas: {resumen.get('criticas', 0)} | Altas: {resumen.get('altas', 0)} | Medias: {resumen.get('medias', 0)} | Bajas: {resumen.get('bajas', 0)}
Pendientes: {resumen.get('pendientes', 0)} | Verificadas: {resumen.get('verificadas', 0)} | Falsos Positivos: {resumen.get('falsos_positivos', 0)} | Confirmadas: {resumen.get('confirmadas', 0)}"""
            
            self.resumen_text.insert("1.0", texto)
        
        self.resumen_text.config(state=tk.DISABLED)
    
    def exportar_vulnerabilidades_ia(self):
        """Exporta las vulnerabilidades detectadas por IA a un archivo Excel"""
        if not hasattr(self, 'vulnerabilidades_detectadas') or not self.vulnerabilidades_detectadas:
            messagebox.showwarning(
                "Sin datos",
                "No hay vulnerabilidades detectadas para exportar. Ejecuta el análisis primero."
            )
            return
        ...
        try:
            from datetime import datetime
            import openpyxl
            from excel_generator import crear_hoja_vulnerabilidades_ia
            
            # Solicitar ubicación para guardar
            archivo = filedialog.asksaveasfilename(
                defaultextension=".xlsx",
                filetypes=[("Excel files", "*.xlsx"), ("All files", "*.*")],
                title="Guardar Vulnerabilidades IA"
            )
            ...
            # Crear hoja de vulnerabilidades IA
            crear_hoja_vulnerabilidades_ia(wb, self.vulnerabilidades_detectadas)
            
            # Guardar archivo
            wb.save(archivo)
            
            self.log(f"✅ Vulnerabilidades IA exportadas: {archivo}")
            messagebox.showinfo(
                "Exportación exitosa",
                f"Vulnerabilidades exportadas exitosamente a:\n{archivo}"
            )
            
        except Exception as e:
            self.log(f"❌ Error al exportar: {str(e)}")
            messagebox.showerror("Error", f"Error al exportar vulnerabilidades:\n{str(e)}")
    
    def setup_phase_vulnerabilities(self):
        """Configura la fase 4: Análisis de Vulnerabilidades"""
        container = ttk.Frame(self.notebook)
        self.notebook.add(container, text="Fase 4: Análisis IA")
        container.columnconfigure(0, weight=1)
        container.rowconfigure(0, weight=1)
        ...
        self.btn_exportar_ia = ttk.Button(
            action_frame,
            text="📊 Exportar a Excel",
            command=self.exportar_vulnerabilidades_ia
        )
        self.btn_exportar_ia.grid(row=0, column=2, padx=5)
        ...
        self.resumen_text = scrolledtext.ScrolledText(resumen_frame, height=4, wrap=tk.WORD)
